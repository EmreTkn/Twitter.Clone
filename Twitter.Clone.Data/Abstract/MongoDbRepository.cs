...
public abstract class MongoDbRepository<T> : IRepository<T, string> where T : MongoDbEntity, new()
{
    ...
    protected MongoDbRepository(IAppSettings settings)
    {
        _settings = settings;
        var client = new MongoClient(_settings.MongoDbConnectionString);
        var db = client.GetDatabase(_settings.MongoDbDatabase);
